...
import { ventaDetalleRepository, ventaRepository } from "@/repositories/venta.repository";
import { stockRepository } from "@/repositories/stock.repository";
import { cajaRepository, cajaMovimientoRepository } from "@/repositories/caja.repository";
import { prisma } from '@config/database';
import { IVenta, ICreateVentaInput, TipoVenta, EstadoVenta } from '@/types/venta.types';
...
export class VentaService {
    ...
    private async validarCreacionProducto(producto, detalle): Promise<void> {
        ...
        if (cantidadStock.lessThan(cantidadVenta)) {
            throw new Error(`Stock insuficiente para el producto ${producto.nombre}. Disponible: ${cantidadStock.toString()}, Solicitado: ${cantidadVenta.toString()}`);
        }
    }
    ...
    async createVenta(data: ICreateVentaInput, user: any): Promise<IVenta> {
        ...
        const venta = await prisma.$transaction(async (tx) => {
            ...
            const venta = await ventaRepository.create({
                ...
                estado: data.tipoVenta === TipoVenta.contado ? EstadoVenta.pagada : EstadoVenta.pendiente,
            ...
            for (const detalle of detallesValidados) {
                ...
                if (stockActual) {
                    const nuevaCantidad = new Decimal(stockActual.cantidad).minus(detalle.cantidad);
            ...
            if (data.tipoVenta === TipoVenta.contado || data.tipoVenta === TipoVenta.transferencia) {
                const medioPagoId = data.tipoVenta === TipoVenta.contado ? 1 : 2;
                const referencia = data.tipoVenta === TipoVenta.contado ? 'Pago contado' : 'Pago por transferencia';
        ...
        if (data.tipoVenta === TipoVenta.contado || data.tipoVenta === TipoVenta.transferencia) {
            try {
                ...
                if (cajaAbierta) {
                    ...
                    console.warn(`⚠️ Usuario ${data.usuarioId} no tiene caja abierta. Venta ${venta.numeroVenta} no se registró en caja.`);
                }
            } catch (error) {
