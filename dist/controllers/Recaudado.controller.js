...
exports.recaudadoController = exports.RecaudadoController = void 0;
const database_1 = require("@config/database");
class RecaudadoController {
    // Controlador para obtener lo recaudado por mes
    async getRecaudadoPorMes(req, res) {
        try {
            const year = req.query.year
                ? parseInt(req.query.year)
                : new Date().getFullYear();
            const recaudadoPorMes = await database_1.prisma.$queryRaw `
        SELECT 
    ...
    async getRecaudadoPorSemana(req, res) {
        try {
            const weeksBack = req.query.weeks
                ? parseInt(req.query.weeks)
                : 12;
            const recaudadoPorSemana = await database_1.prisma.$queryRaw `
        SELECT 
...
exports.RecaudadoController = RecaudadoController;
exports.recaudadoController = new RecaudadoController();
