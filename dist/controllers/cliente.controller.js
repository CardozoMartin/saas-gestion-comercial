...
class ClienteController {
    async getAllClientes(req, res) {
        try {
            const clientes = await cliente_service_1.clienteService.getAllClientes();
    ...
    async getClienteById(req, res) {
        try {
            ...
            const cliente = await cliente_service_1.clienteService.getClienteById(id);
    ...
    async createCliente(req, res) {
        try {
            const cliente = await cliente_service_1.clienteService.createCliente(req.body);
    ...
    async updateCliente(req, res) {
        try {
            const id = parseInt(req.params.id);
            const cliente = await cliente_service_1.clienteService.updateCliente(id, req.body);
            return res.status(200).json(cliente);
        }
        catch (error) {
            return res.status(500).json({ error: error.message });
    ...
    async deleteCliente(req, res) {
        try {
            const id = parseInt(req.params.id);
            await cliente_service_1.clienteService.deleteCliente(id);
    ...
    async getResumenCuentaCorriente(req, res) {
        try {
            const clienteId = parseInt(req.params.id);
            const resumen = await cliente_service_1.clienteService.getResumenCuentaCorriente(clienteId);
    ...
    async getDetalleVenta(req, res) {
        try {
            ...
            const detalle = await cliente_service_1.clienteService.getDetalleVenta(ventaId);
    ...
    // MÉTODOS DE PAGO
    async pagarCuentaCorriente(req, res) {
        try {
            const clienteId = parseInt(req.params.id);
            ...
            const resultado = await cliente_service_1.clienteService.pagarCuentaCorriente({
                clienteId,
    ...
    async pagarVentasEspecificas(req, res) {
        try {
            const clienteId = parseInt(req.params.id);
            ...
            const resultado = await cliente_service_1.clienteService.pagarVentasEspecificas({
                clienteId,
                ventasAPagar,
                medioPagoId: parseInt(medioPagoId),
                referencia,
    ...
    async getClientesConMayorDeuda(req, res) {
        try {
            const limit = req.query.limit ? parseInt(req.query.limit) : 10;
            const clientesConDeuda = await database_1.prisma.$queryRaw `
        SELECT 
          c.id,
    ...
    async toggleClienteActivo(req, res) {
        const idCliente = parseInt(req.params.id);
        const user = req.user;
        try {
            const updatedCliente = await cliente_service_1.clienteService.changeStatus(idCliente, user);
            return res.status(200).json({
