...
class UnitConversionService {
     ...
     */
    convertirAUnidadBase(cantidad, unidadAbreviatura) {
     ...
     */
    convertirDesdeUnidadBase(cantidadBase, unidadAbreviatura) {
        const unidadNormalizada = this.normalizarUnidad(unidadAbreviatura);
        const factor = this.CONVERSION_FACTORS[unidadNormalizada];
        if (factor === undefined) {
            throw new Error(`Unidad de medida no soportada: "${unidadAbreviatura}" (normalizada: "${unidadNormalizada}"). ` +
     ...
     */
    convertir(cantidad, unidadOrigen, unidadDestino) {
     ...
     */
    sonUnidadesCompatibles(unidad1, unidad2) {
        ...
        const compatible = ((tiposPeso.includes(u1) && tiposPeso.includes(u2)) ||
     ...
     */
    obtenerTipoUnidad(unidad) {
        const u = this.normalizarUnidad(unidad);
        if (['g', 'gr', 'kg', 'kgs', 'lb', 'lbs', 'oz'].includes(u))
            return 'peso';
        if (['ml', 'cc', 'l', 'lt', 'lts', 'litro', 'litros', 'gal', 'galon'].includes(u))
            return 'volumen';
        if (['mm', 'cm', 'm', 'mts'].includes(u))
            return 'longitud';
        if (['unidad', 'un', 'caja', 'paquete', 'paq'].includes(u))
            return 'unidad';
        return 'desconocido';
    }
}
