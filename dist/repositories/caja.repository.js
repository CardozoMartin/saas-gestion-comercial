...
exports.cajaMovimientoRepository = exports.cajaRepository = exports.CajaMovimientoRepository = exports.CajaRepository = void 0;
...
class CajaRepository {
    async findAll() {
        return await database_1.prisma.caja.findMany({
            select: {
                ...
                montoInicial: true,
                montoFinal: true,
                montoFinalContado: true, // ✅ NUEVO
                montoRetirado: true, // ✅ NUEVO
                fondoSiguienteCaja: true, // ✅ NUEVO
                cajaAnteriorId: true, // ✅ NUEVO
                totalEfectivo: true,
    ...
    async findById(id) {
        return await database_1.prisma.caja.findUnique({
            where: { id },
            select: {
                id: true,
                usuarioId: true,
    ...
    async findByUsuarioIdAndEstado(usuarioId, estado) {
        return await database_1.prisma.caja.findFirst({
    ...
    async create(data) {
        return await database_1.prisma.caja.create({
            data: {
    ...
    async update(id, data) {
        return await database_1.prisma.caja.update({
            where: { id },
            data,
            select: {
                id: true,
    ...
    async delete(id) {
        await database_1.prisma.caja.delete({
            where: { id }
        });
    }
    async findResumenCajaAbierta(usuarioId) {
        const cajaAbierta = await database_1.prisma.caja.findFirst({
            where: {
                usuarioId,
                estado: 'abierta'
            },
        ...
        const cantidadProductosVendidos = ventas.reduce((sum, v) => sum + v.detalles.reduce((detSum, d) => detSum + Number(d.cantidad), 0), 0);
...
class CajaMovimientoRepository {
    async findAll() {
        return await database_1.prisma.cajaMovimiento.findMany({
            select: {
                id: true,
                cajaId: true,
                pagoId: true,
    ...
    async findById(id) {
        return await database_1.prisma.cajaMovimiento.findUnique({
            where: { id },
            select: {
                id: true,
                cajaId: true,
    ...
    async create(data) {
        return await database_1.prisma.cajaMovimiento.create({
            data: {
                cajaId: data.cajaId,
    ...
    async update(id, data) {
        return await database_1.prisma.cajaMovimiento.update({
            where: { id },
            data,
            select: {
                id: true,
    ...
    async delete(id) {
        await database_1.prisma.cajaMovimiento.delete({
            where: { id }
        });
    }
}
exports.CajaMovimientoRepository = CajaMovimientoRepository;
exports.cajaRepository = new CajaRepository();
exports.cajaMovimientoRepository = new CajaMovimientoRepository();
